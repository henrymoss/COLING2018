...
################################
#script to produce figure 1(a)
#input: none
#output: figure showing variation in train/split
#        prediction error estimates for different split proportions
##############################
...
#repeat train/test split of fraction 1/k
def repeat(numberofreplications,k):
	out=[]
	for j in range(0,numberofreplications):
		print(j)
		X_train, X_test,Y_train,Y_test=train_test_split(X_fixed_tfidf,Y_fixed,test_size=1/k,random_state=rs+j*k)
		classif.fit(X_train,Y_train)
		scores=accuracy_score(Y_test, classif.predict(X_test))
		out.append(scores)
	return out
#run experiment 
...
out=[]
for i in range(2,15):
	out.append(repeat(numberofrepetitions,i))
	print(i)
...
plt.xlabel('Fraction of data used for testing')
plt.ylabel('Accuracy')
plt.ylim(0.55,0.85)
plt.axhline(y=0.7285714285714285,color="r")
plt.show()
plt.savefig('MOVIES_VAR_Hold_Out_2.pdf')
